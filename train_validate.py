...
class TrainerValidator:
	...
	def __init__(self, k, nb_epochs, H1, H2, nu, mu, test_set_size, validation_set_size):
		...
		self.validation_error = sp.zeros(self.NUM_EPOCH)
		self.misclassified_val = sp.zeros(self.NUM_EPOCH)
		self.training_error = sp.zeros(self.NUM_EPOCH)
		self.misclassified_train = sp.zeros(self.NUM_EPOCH)
	...
	def trainAndClassify(self):
		for i in range(self.NUM_EPOCH+1):
			...
			if i != self.NUM_EPOCH :
				self.validation_error[i], self.misclassified_val[i] = self.error.total_error(results_val, self.data.val_cat, self.k)
				print "Epoch #"+str(i+1)+" Ratio of misclassified: "+str(self.misclassified_val[i])+" - Logistic error: "+str(self.validation_error[i])
			if i != 0 :
