...
def main():
	...
	for i in range(NUM_EPOCH):
		...
		#print "-"*30 + " Error " + "-"*30
		validation_error[i], misclassified_val[i] = error.total_error(results_val, data.val_cat)
		training_error[i], misclassified_train[i] = error.total_error(results_train, data.train_cat)
